      ...
      if (weeks[days] === nowDay) {
        for (let i = 1; i <= dateCount; i++) {
          ...
          today.year === select.year && today.month === select.month && today.day === i
            ? dayArr.push(
                <Today value={i} key={i} onClick={dateSelect}>
                  {i}
                </Today>
              )
            : day === 0
            ? dayArr.push(
                <Sunday value={i} key={i} onClick={dateSelect}>
                  {i}
                </Sunday>
              )
            : day === 6
            ? dayArr.push(
                <Saturday value={i} key={i} onClick={dateSelect}>
                  {i}
                </Saturday>
              )
            : dayArr.push(
                <Day value={i} key={i} onClick={dateSelect}>
                  {i}
                </Day>
              );
        }
